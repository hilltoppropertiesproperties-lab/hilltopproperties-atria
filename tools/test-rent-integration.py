"""Rent integration regression checks. Run from any directory with Python Playwright installed.
...
OUT = ROOT / '.codex-artifacts' / 'rent-integration'
...
def property_row(n, **changes):
    row = dict(id=f'00000000-0000-0000-0000-{n:012d}', reference_number=f'TEST-{n}',
               ...
               currency_code='ZMW', purpose='For Rent', property_type='House',
...
fixtures = [property_row(1), property_row(2, status='Under Offer', property_type='Apartment', bedrooms=2),
            ...
            property_row(4, purpose='For Sale'), property_row(5, status='Draft'),
...
url = base + '/property%20replica/rent/listings.html'
report = {'checks': [], 'live': {}}
...
def install_api(page, mode='normal', rows=None):
    ...
    def api(route):
        ...
        if parsed.path.endswith('/properties'):
            check('query enforces Rent', query.get('purpose') == ['eq.For Rent'])
            ...
            if mode != 'invalid-response':
                data = [r for r in data if r['purpose'] == 'For Rent' and r['status'] in ('Active', 'Under Offer')]
            ...
            check('images restricted to returned Rent IDs', set(ids) <= {r['id'] for r in (expected if rows is None else rows)})
            if mode == 'image-error':
            ...
            data = [dict(property_id=expected[0]['id'], image_url=base+'/assets/images/hero-poster.png', display_order=2, is_cover=False),
                    dict(property_id=expected[0]['id'], image_url=base+'/property%20replica/rent/assets/images/hilltop-logo.png', display_order=9, is_cover=True),
...
try:
    with sync_playwright() as p:
        ...
        ready(page)
        report['live']['rent_count'] = count(page)
        ...
        check('live page has only Rent badges', set(page.locator('.purpose-badge').all_text_contents()) <= {'For Rent'})
        ...
        report['live']['quick_filters'] = page.locator('#quickFilterOptions button').all_text_contents()
        check('live quick filters match live locations', sorted(report['live']['quick_filters']) == sorted(page.locator('#listingLocations option').evaluate_all('(options)=>options.map(o=>o.value)')))
        ...
        for width in [1440, 1024, 900, 600, 390, 360]:
            ...
            if width in (1440, 390):
                page.screenshot(path=str(OUT / f'rent-live-{width}.png'), full_page=False)
        ...
        page.goto(url+'?purpose=sale&listingType=sale', wait_until='domcontentloaded')
        ready(page)
        check('Active and Under Offer shown; Sale and other statuses absent', count(page) == 3)
        check('URL purpose cannot change Rent', page.locator('#listingPurposeFilter').input_value() == 'For Rent')
        page.evaluate("() => { history.pushState({}, '', '?purpose=sale'); dispatchEvent(new PopStateEvent('popstate')); }")
        check('browser state keeps the dedicated Rent purpose', page.locator('#listingPurposeFilter').input_value() == 'For Rent' and set(page.locator('.purpose-badge').all_text_contents()) == {'For Rent'})
        ...
        check('transaction Sale control prepares Sale routing', page.locator('#listingPurposeFilter').input_value() == 'For Sale' and count(page) == 3)
        ...
        clear(page)
        check('Clear filters retains fixed purpose', count(page) == 3 and page.locator('#listingPurposeFilter').input_value() == 'For Rent')
        ...
        check('averages separate currencies', page.locator('.listings-lower-prices tbody tr').first.locator('td').inner_text() == 'ZMW K150,000 / month / USD $50,000 / month')
        ...
        check('mobile Clear Filters reachable', page.locator('#resetFilters').is_visible())
        page.locator('#resetFilters').click()
        check('mobile Clear Filters retains Rent', count(page) == 3 and page.locator('#listingPurposeFilter').input_value() == 'For Rent')
        check('existing monthly fallback', all('/ month' in text for text in page.locator('.property-price').all_text_contents()))
        page.screenshot(path=str(OUT/'rent-mobile-filters.png'), full_page=False)
        page.close()
        ...
        if baseline_html.exists() and baseline_js.exists():
            ...
            for variant in ['before', 'after']:
                page = browser.new_page(viewport={'width': 1440, 'height': 1100})
                if variant == 'before':
                    page.route('**/rent/listings.html', lambda r: r.fulfill(content_type='text/html', body=baseline_html.read_text(encoding='utf-8')))
                    page.route('**/rent/listings-ui.js*', lambda r: r.fulfill(content_type='application/javascript', body=baseline_js.read_text(encoding='utf-8')))
                ...
                for width in [1440, 390]:
                    ...
                    page.screenshot(path=str(OUT / f'rent-{variant}-{width}.png'))
                page.close()
        ...
        for filename in ['listings.css', 'listings-filter.css']:
            check(filename+' unchanged', (OUT/('before-'+filename)).read_bytes() == (ROOT/'property replica'/'rent'/filename).read_bytes())
