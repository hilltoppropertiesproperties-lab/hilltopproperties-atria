...
data.properties = data.properties.filter(row => row.purpose === 'For Rent');
const rentalIds = new Set(data.properties.map(row => String(row.id)));
// Bundled rental examples use monthly prices; preserve an explicit period when supplied.
for (const property of data.properties) property.billing_period ||= 'month';
data.images = data.images.filter(row => rentalIds.has(String(row.property_id)) && !row.image_url.includes('example.com'));
