/* Rent presentation and client-side filters over public Supabase inventory. */
(function () {
  'use strict';
  ...
  var rentalPurpose = 'For Rent';
  // Live schema has no billing period. The existing Rent formatter defaults to month.
  ...
  function listingSearchParams() {
    ...
    params.set('purpose', normalizePurpose(byId('listingPurposeFilter').value) || 'rent');
  ...
  function readListingSearchParams() {
    ...
    byId('listingPurposeFilter').value = rentalPurpose;
  ...
  function routeListingSearch() {
    var purpose = normalizePurpose(byId('listingPurposeFilter').value) || 'rent';
    var params = listingSearchParams();
    if (purpose === 'sale') {
      var target = new URL('../sales/listings.html', window.location.href);
  ...
  function renderProperties(properties) {
    ...
    return '<article class="property-card" aria-labelledby="property-title-' + index + '">' +
      ...
      '<div class="property-info"><p class="property-price">' + (property.price == null ? 'Price on request' : window.HilltopCurrency.formatPropertyPrice(property.price,property.currency_code,property.purpose,property.billing_period)) + '</p><h2 class="property-title" id="property-title-' + index + '"><a href="' + url + '">' + title + '</a></h2><p class="property-location">' + [property.area, property.property_type].filter(Boolean).map(escape).join(' · ') + '</p>' + (specifications.length ? '<div class="property-specs">' + specifications.join('') + '</div>' : '') + '</div></article>';
    }).join('');
    if (!properties.length) grid.innerHTML = '<div class="listings-empty"><h2>' + (allProperties.length ? 'No properties found' : 'No properties for rent yet') + '</h2><p>' + (allProperties.length ? 'Try clearing one or more filters.' : 'Please check back soon for new Rent listings.') + '</p></div>';
  ...
  function applyFilters() {
    ...
    byId('listingPageTitle').textContent = 'Properties to Rent';
    document.querySelector('.breadcrumb a').textContent = 'Property to Rent';
    return properties;
  }
  ...
  function placeControls() {
    ...
    // Preserve Rent's reachable mobile Clear filters control.
    if (mobile.matches) byId('advancedFilters').appendChild(byId('resetFilters'));
    else dialog.querySelector('.dialog-footer').insertBefore(byId('resetFilters'), byId('applyFilters'));
  ...
  byId('resetFilters').addEventListener('click',function () { byId('listingSearchForm').reset(); byId('listingPurposeFilter').value = 'For Rent'; syncSelectControls(); byId('quickFilterOptions').querySelectorAll('button').forEach(function (item) { item.setAttribute('aria-pressed','false'); }); applyFilters(); });
  ...
  document.addEventListener('click',function (event) {
    ...
    if (demo) toast(demo.dataset.preview === 'For Sale' ? 'Sales navigation will be connected in the routing stage.' : demo.dataset.preview + ' is not connected yet.');
  ...
  function renderAverages() {
    ...
    document.querySelectorAll('.listings-lower-prices tbody tr').forEach(function (row, index) {
      var prices = ['ZMW', 'USD'].map(function (currency) {
        ...
        return currency + ' ' + window.HilltopCurrency.formatPropertyPrice(Math.round(average * 100) / 100, currency, rentalPurpose);
      }).filter(Boolean);
      row.querySelector('td').textContent = prices.join(' / ') || 'Not available';
    });
    byId('averagePriceNote').textContent = allProperties.length
      ? 'Average asking prices across current Rent listings, shown separately by currency. Search filters do not change these averages. Billing periods are unavailable; / month uses the existing display fallback.'
  ...
  function showInventoryState(state) {
    ...
    grid.innerHTML = '<div class="listings-empty"><h2>' + (loading ? 'Loading properties for rent…' : 'Properties could not be loaded') +
    ...
    byId('filterMatchingCount').textContent = loading ? 'Loading Rent properties…' : 'Rent properties are temporarily unavailable.';
  ...
  async function loadInventory() {
    ...
    try {
      data = await window.HilltopListingsData.loadPublicListings(rentalPurpose);
      ...
      console.warn('Rent inventory could not be loaded:', error.message || error);
