...
The active `listings.html` is rental-only and uses two bundled sample properties. Sales controls navigate to `../sales/listings.html`, which exists in the sibling Sales folder. Serve the parent directory when verifying cross-page navigation.

Run `node tools/build-listings-preview.cjs` to regenerate rental properties and their images, then `python tools/review-rent.py` for the current regression suite. It starts its own local server. New results and desktop/mobile screenshots go to `verification/rent/`.

The rental samples explicitly use monthly billing; supplied billing periods are passed through to the shared formatter. The office sample has no valid bundled image and displays the existing photo-unavailable placeholder. The page remains a sample-data preview, with no Supabase connection. URL parameters do not change its rental purpose; category parameters remain unsupported.

The seven earlier review scripts listed below describe previous design stages and are superseded by `review-rent.py` for this route. Some also reference controls removed before this conversion. Their historical assertions and verification artifacts are retained for reference.

- `review-sidebar.py`
- `review-filter-flyouts.py`
- `review-listings.py`
- `review-lower-page.py`
- `review-sidebar-actions.py`
- `review-quick-filters.py`
- `review-quick-carousel.py`

## Historical Sales UI review
