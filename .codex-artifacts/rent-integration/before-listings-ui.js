...
(function () {
  ...
  // This route remains rental-only even if mixed data or a Sales URL parameter is supplied.
  var rentalPurpose = 'For Rent';
  var allProperties = data.properties.filter(function (row) { return row.purpose === rentalPurpose; });
  ...
  var locations = Array.from(new Set(allProperties.map(function (row) { return row.area; })));
  ...
  function placeControls() {
    ...
    // Keep Clear filters reachable in the inline mobile filters as well as the desktop flyout.
  ...
  byId('resetFilters').addEventListener('click',function () { byId('listingSearchForm').reset(); byId('listingPurposeFilter').value = rentalPurpose; syncSelectControls(); byId('quickFilterOptions').querySelectorAll('button').forEach(function (item) { item.setAttribute('aria-pressed','false'); }); applyFilters(); });
  ...
  document.querySelectorAll('[data-listing-purpose]').forEach(function (button) {
    button.addEventListener('click',function () {
      if (button.dataset.listingsRoute) return;
      byId('listingPurposeFilter').value = rentalPurpose;
  ...
  document.addEventListener('click',function (event) {
    var route = event.target.closest('[data-listings-route]');
    if (route) { window.location.assign(route.dataset.listingsRoute); return; }
